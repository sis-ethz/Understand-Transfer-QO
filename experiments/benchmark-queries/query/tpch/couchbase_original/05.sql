...
UPDATE STATISTICS FOR tpch_orders(o_custkey, o_orderkey, o_orderdate);
UPDATE STATISTICS FOR tpch_lineitem(l_orderkey, l_suppkey);
UPDATE STATISTICS FOR tpch_supplier(s_suppkey, s_nationkey);
UPDATE STATISTICS FOR tpch_nation(n_nationkey, n_regionkey);
UPDATE STATISTICS FOR tpch_region(r_regionkey, r_name);
...
FROM tpch_customer
    ...
    JOIN tpch_lineitem ON tpch_lineitem.l_orderkey = tpch_orders.o_orderkey
    JOIN tpch_supplier ON tpch_lineitem.l_suppkey = tpch_supplier.s_suppkey
    JOIN tpch_nation ON tpch_supplier.s_nationkey = tpch_nation.n_nationkey
    JOIN tpch_region ON tpch_nation.n_regionkey = tpch_region.r_regionkey
