...
from
	tpch_lineitem
	join tpch_orders USE HASH(BUILD) on tpch_lineitem.l_orderkey = tpch_orders.o_orderkey
	join tpch_customer USE HASH(BUILD) on tpch_customer.c_custkey = tpch_orders.o_custkey
	join tpch_nation USE HASH(BUILD) on tpch_customer.c_nationkey = tpch_nation.n_nationkey
where
