-- SELECT SUM(lo_revenue), d_year, p_brand1
-- FROM  lineorder, ddate, part, supplier
-- WHERE lo_orderdate = d_datekey
--   AND lo_partkey = p_partkey
--   AND lo_suppkey = s_suppkey
--   AND p_category = 'MFGR#12'
--   AND s_region = 'AMERICA'
-- GROUP BY d_year, p_brand1
-- ORDER BY d_year, p_brand1;

CREATE INDEX temp_ix_1 ON ssb_lineorder(lo_orderdate, lo_partkey, lo_suppkey);
CREATE INDEX temp_ix_2 ON ssb_ddate(d_datekey, d_year);
CREATE INDEX temp_ix_3 ON ssb_part(p_partkey, p_category);
CREATE INDEX temp_ix_4 ON ssb_supplier(s_suppkey, s_region);

UPDATE STATISTICS FOR ssb_lineorder(lo_orderdate, lo_partkey, lo_suppkey);
UPDATE STATISTICS FOR ssb_ddate(d_datekey, d_year);
UPDATE STATISTICS FOR ssb_part(p_partkey, p_category);
UPDATE STATISTICS FOR ssb_supplier(s_suppkey, s_region);


SELECT SUM(ssb_lineorder.lo_revenue),
       ssb_ddate.d_year,
       ssb_part.p_brand1
FROM ssb_lineorder
INNER JOIN ssb_ddate ON ssb_lineorder.lo_orderdate = ssb_ddate.d_datekey
INNER JOIN ssb_part ON ssb_lineorder.lo_partkey = ssb_part.p_partkey
INNER JOIN ssb_supplier ON ssb_lineorder.lo_suppkey = ssb_supplier.s_suppkey
WHERE ssb_part.p_category = 'MFGR#12'
    AND ssb_supplier.s_region = 'AMERICA'
GROUP BY ssb_ddate.d_year,
         ssb_part.p_brand1
ORDER BY ssb_ddate.d_year,
         ssb_part.p_brand1;
         
         
DROP INDEX ssb_lineorder.temp_ix_1;
DROP INDEX ssb_ddate.temp_ix_2;
DROP INDEX ssb_part.temp_ix_3;
DROP INDEX ssb_supplier.temp_ix_4;
